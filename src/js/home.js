...
// get card elements from db start
...
.then(cavab => {
    ...
    function hasDiscount(e){
        ...
        let newCard = `<div class="card" data-id="${element.id}">
        <div class="card-sections">
            <div class="card-img-sec">
                <p class="about-item">${newOld(element.isNew)}</p>
                <p class="item-endirim">${hasDiscount(element.endirim)}</p>
                <img class="item-img" src="${element.img[0]}" alt="${element.itemName}">
                <div class="img-hover-item">
                    <p id="quick-look" onclick="quickLookModal(this)">QUICK LOOK</p>
                    <span>&#9825;</span>
                </div>
            ...
            <div class="card-text-sec">
                ...
                <div class="hover-text">
                    <p class="add-to-cart-text" data-id="${element.id}" onclick="addToCartCardItem(this)">ADD TO CART</p>
                    <p class="item-price"><span class="old-price">${ifEndirim(element.endirim,element.itemPrice)}</span> ${endirim(element.endirim,element.itemPrice)}</p>
                </div>
